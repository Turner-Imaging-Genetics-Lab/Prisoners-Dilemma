...
expName = 'PrisonersdilemmaToneRilling2'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\khalilt93\\Desktop\\PsychoPyDemos\\prisonersdilemmatask\\PrisonersdilemmaToneRilling2.py',
...
instructions1 = visual.TextStim(win=win, name='instructions1',
    text='Welcome to the Prisoners Dilemma Game!\n',
    font='Arial',
...
instructions2 = visual.TextStim(win=win, name='instructions2',
    text='You must decide whether you want to cooperate or not cooperate with your partner.\n\n',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color='black', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
instructions3 = visual.TextStim(win=win, name='instructions3',
    text='During gameplay, you will press C to cooperate and D to not cooperate.\n',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color='black', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
instructions4 = visual.TextStim(win=win, name='instructions4',
    text='Based on the combination of your decisions you both will win specific amounts of money!',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color='black', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
instructions5 = visual.TextStim(win=win, name='instructions5',
    text='You will play 20 rounds with each other total before final earnings are tallied.\n\n',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color='black', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
roundsloop = data.TrialHandler(nReps=9999, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('stimuli\\PDstimuli.xlsx', selection='0'),
    seed=None, name='roundsloop')
thisExp.addLoop(roundsloop)  # add the loop to the experiment
thisRoundsloop = roundsloop.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisRoundsloop.rgb)
if thisRoundsloop != None:
    for paramName in thisRoundsloop:
        exec('{} = thisRoundsloop[paramName]'.format(paramName))

for thisRoundsloop in roundsloop:
    currentLoop = roundsloop
    # abbreviate parameter names if possible (e.g. rgb = thisRoundsloop.rgb)
    if thisRoundsloop != None:
        for paramName in thisRoundsloop:
            exec('{} = thisRoundsloop[paramName]'.format(paramName))
    ...
    phaseinstruct.setText('Choose if you want to cooperate (C) or not cooperate (D)')
    phaseinstruct1_2.setColor('black', colorSpace='rgb')
    ...
    if key_decision.keys == 'c':
        ...
        if player1 == "d" and player2 == "d":
            ...
            outcomes += ["dd"]
            outcomesExc = "dd"
    elif key_decision.keys not in decision:
        doDecision2 = 1
        
    #data to be added to Excel
    thisExp.addData("computer_decision",player2)
    
    roundsloop.addData('Decision_Screen1.started', Decision_Screen1.tStartRefresh)
    roundsloop.addData('Decision_Screen1.stopped', Decision_Screen1.tStopRefresh)
    # check responses
    if key_decision.keys in ['', [], None]:  # No response was made
        key_decision.keys = None
    roundsloop.addData('key_decision.keys',key_decision.keys)
    if key_decision.keys != None:  # we had a response
        roundsloop.addData('key_decision.rt', key_decision.rt)
    roundsloop.addData('key_decision.started', key_decision.tStartRefresh)
    roundsloop.addData('key_decision.stopped', key_decision.tStopRefresh)
...
# completed 9999 repeats of 'roundsloop'
